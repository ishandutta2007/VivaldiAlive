...
class Settings:
    ...
    # locations
    IS_RUNNING = True
    log_location = localize_path("VivaldiAlive", "logs")
    database_location = localize_path("VivaldiAlive", "db")
    OS_ENV = (
    ...
    chromedriver_location = localize_path(
        "VivaldiAlive", "assets", specific_chromedriver
    )
    if not chromedriver_location or not path_exists(chromedriver_location):
        chromedriver_location = localize_path("VivaldiAlive", "assets", "chromedriver")

    # minimum supported version of chromedriver
    chromedriver_min_version = 2.36

    platform_name = "vivaldialive"

    # set a logger cache outside the VivaldiAlive object to avoid
    ...
    WORKSPACE = {"name": "VivaldiAlive", "path": environmental_variables.get("HOME")}

    DATABASE_LOCATION = localize_path("VivaldiAlive", "db", "vivaldialive.db")
